...
## [Unreleased] - Quality Sprint

### セキュリティ

- **APIキーをセッション限定に変更** (#41, PR #45): デフォルトでAPIキーをsessionStorageに保存
  - 設定でオプトインすれば従来通りlocalStorageに永続化可能
  - 既存ユーザー向けにlocalStorage→sessionStorage移行フローを追加
- **ログのAPIキー秘匿化** (#40, PR #45): エラーログ出力時にAPIキーパターンを自動マスク

### 信頼性 / UX

- **致命的エラー復旧UI** (#40, PR #45): グローバルエラーハンドラー追加
  - 「セッションリセット」「ページ再読み込み」の復旧オプションを提供
- **文字起こし描画最適化** (#44, PR #45): マーカーインデックス検索のループ回避
- **マイクストリーム重複防止** (#32, PR #45): 既存ストリームの適切なクリーンアップ
- **イベント二重バインド防止** (#33, PR #45): 重複リスナー登録を回避
- **SecureStorage堅牢化** (#36, PR #45): JSONパースエラー時のフォールバック処理

### コンプライアンス / ビルド

- **THIRD_PARTY_NOTICES更新** (#42, PR #45): 上流ライセンス・配布元の明記
- **CIスモークチェック追加** (#43, PR #45): 最小限のCIワークフローと権限設定

### 動作変更に関する注意

- **デフォルト動作変更**: APIキーはセッション終了時に削除されます。永続化したい場合は設定で「APIキーを次回も保持する」をONにしてください。(#41)
