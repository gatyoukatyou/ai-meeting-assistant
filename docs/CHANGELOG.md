...
### Changed
- **ローカルLLMのタイムアウト延長**: local_llm 選択時のみ AI機能のタイムアウトを 30秒→180秒（クラウド API は30秒のまま）。実測 gpt-oss:20b (Mac mini) で会議要約 30〜90秒/回 (#221)
- **Local LLM thinking 制御**: OpenAI互換ローカルサーバー（Ollama / LM Studio）に対し、要約時は thinking を無効化し、boost 付き相談時のみ有効化 (#219)

### Fixed
- **タイムアウト文言**: 固定の「30秒」表記を除去（ja/en）。タイムアウト秒数はプロバイダ依存のため (#221)

### Known Notes
- Ollama 0.33.0 の `/v1` は `think` フィールドを無視するため、thinking モデル（qwen3.5 等）は低速のまま。Ollama 側の llama.cpp 同期待ち
- LM Studio + 27B クラスのモデルは ~6 tok/s。実会議プロンプトの要約が180秒を超える場合は、GUI で reasoning を無効化するか 20B 以下のモデルを推奨
- 複数ランタイムへの同時ロード（合計20GB超）はメモリ圧迫で推論速度が激減するため、ロードは1モデルに絞ることを推奨

---

## v1.5.0 (2026-08-11) - clean テーマ刷新 & 既定テーマ切替

### Added
- **`#styleSwitcher` に clean / CLI を追加**: メインページからもテーマ切替が可能に

### Changed
- **clean テーマ刷新**: ダーク基調モダン(Linear dark 風)へ再設計。アクセントカラー9色が `btn-primary` 等へ正しく反映されるよう修正
- **既定テーマ変更**: 新規ユーザーは `clean` が既定に。既存ユーザーの localStorage 保存値は上書きされない
- **モバイル下部タブ**: clean では非アクティブを控えめな配色、アクティブのみアクセントカラー表示に変更

### Fixed
- **clean `.btn-primary`**: `#111827` 直書きを廃止し `var(--primary)` トークンを使用(ダーク背景での視認性問題を解消)
- **入力欄の inset 影**: clean では box-shadow を無効化
- **ダッシュ線/スクロールバー**: clean では実線化・細身のスクロールバーに統一

### Added (2026-09-04 リリース時追記)
- **Local LLM サポート (Phase 1)**: Ollama / LM Studio の localhost エンドポイントに対応。APIキー不要・費用0円 (#211)
- **設定ページ修復**: Local LLM の接続テスト/モデル取得に必要な model-registry を config ページへ読込 (#215)
- **プリセット鮮度チェック**: プロバイダープリセットが90日以上未検証の場合に CI を失敗させる (#217)
