...
- ✅ APIキーは**既定でセッション内のみ保持**（タブ/ブラウザを閉じると削除）
- ✅ デスクトップアプリ（Chrome/Edgeアプリ）では、明示ON時のみ端末保存を許可（非推奨）
- ✅ 通常機能では開発者の外部サーバーへAPIキーを送信しません（AIサービスへの直接通信のみ）
...
- **共有PCでは「APIキー記憶」をOFFにし、「ブラウザを閉じたら削除」をONにしてください**
...
### Realtime音声の30〜60秒テスト（ローカル専用）

Realtime音声テストは、通常の静的配信とは別に、ループバックアドレスだけで動くNode.jsサーバーを起動します。標準OpenAI APIキーはこのローカルプロセスだけが環境変数から読み、ブラウザへは短命なクライアントシークレットだけをメモリ上で渡します。GitHub Pagesや公開サーバーでは実行しないでください。

1. Chrome系のデスクトップブラウザとヘッドホンを用意する
2. プロジェクトフォルダで、キー値をコマンド引数やファイルへ書かずに入力する
   ```bash
   read -s OPENAI_API_KEY
   export OPENAI_API_KEY
   npm run start:realtime
   ```
3. `http://127.0.0.1:8787` を開き、「Realtime音声テスト」の「音声テスト開始」を押す
4. マイクを許可し、日本語で短い会話を30〜60秒行う
5. AI音声、タイムラインのAI回答、画面の使用量と概算を確認して「終了」を押す
6. 終了後、必要ならターミナルで `unset OPENAI_API_KEY` を実行する

この操作はOpenAI Realtime APIの利用料が発生します。単価は画面に表示された参照日時の公式値による概算で、実際の請求額とは異なる場合があります。スクリーンショットやIssue/PRへAPIキー、シークレット、音声内容を貼り付けないでください。エラー共有時は画面の一般化されたエラー、発生時刻、ブラウザ名だけを共有してください。iOS Safari、長時間接続、本番公開は今回の対象外です。
...
**Q: APIキーが漏れたら危ないですか？**
A: はい。他の人があなたのキーを使って、あなたに料金が請求される可能性があります。APIキーは既定でセッション限定ですが、デスクトップアプリでは任意で端末保存をONにできます。共有PCでは必ず記憶をOFFにし、タブ/ブラウザを閉じて手動削除してください。
...
**v1.6.0** - ローカルLLM実用化 & リアルタイムSTT拡充 & 録音中プロファイル切替（2026-09-05）

- Ollama / LM Studio など API キー不要のローカル LLM で AI 機能が利用可能に（#211）
- OpenAI Realtime Transcribe を追加。既存の OpenAI キーでリアルタイム文字起こし（#223）
